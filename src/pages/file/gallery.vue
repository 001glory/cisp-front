    ...
    <div>
        ...
        <div class="panel-start">
            <div class="gallery_meun" style="height:500px;overflow-y:scroll;overflow-x:hidden">
                <el-button style="margin:10px 0" icon="el-icon-circle-plus-outline" @click="addDialog = true,acStatus = true,submitData.groupName=''">添加分类</el-button>
                <el-menu style="width:150px"
                    ...
                    <el-menu-item v-for="(item,index) in cates" :index="index+'-'+item.groupName" :key="index">
            ...
            <div class="gallery-panel">
                    ...
                    <div class="panel-start">
                        ...
                        <el-dropdown style="margin-left:10px">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item v-for="(item,index) in cates" :key="index"><div  @click="moveTo(item.id)">{{item.groupName}}</div></el-dropdown-item>
                            </el-dropdown-menu>
                        </el-dropdown>
                    </div>
                ...
                <div class="kupa">
                    ...
                    <div class="kupa-item"  v-for="(item,index) in list" :key="index" @click="checkIt(index)">
                        <div class="kuit-img" :style="'background-image:url(../../../assets/img/qrcode.jpg)'"></div>
                        <div class="kuit-txt">
                            <div>{{item.mimetype ? item.mimetype.replace('image/',''):''}}</div>
                            <div>{{(item.fileSize/1024)/1024 >= 1 ? (item.fileSize/1024/1024).toFixed(0) +'mb': (item.fileSize/1024).toFixed(0)+'kb' }}</div>
                        </div>
        ...
        <el-dialog :title="acStatus ? '添加分类':'修改分类'" :visible.sync="addDialog" width="300px" center>
            <span><el-input v-model="submitData.groupName" placeholder="分类名称"  /></span>
