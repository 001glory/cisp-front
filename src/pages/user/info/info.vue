    ...
    <div style="margin:auto">
      ...
      >
        ...
        <el-table-column prop="userSy" label="用户收益">
          <template slot-scope="scope">
            <span>{{scope.row.userSy*100 + '%'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="dlSy" label="代理收益">
          <template slot-scope="scope">
            <span>{{scope.row.dlSy*100 + '%'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="psy" label="平台收益">
          <template slot-scope="scope">
            <span>{{scope.row.psy*100 + '%'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="isShow" label="是否开启">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isShow" :active-value="1" :inactive-value="0"></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="priceGui" label="价格"></el-table-column>
        <el-table-column prop="des" label="描述"></el-table-column>
        <el-table-column prop="jdr"  label="默认接单人">
          <!--<template slot-scope="scope">-->
            <!--<div>-->
