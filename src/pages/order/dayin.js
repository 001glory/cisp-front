...
let list = {
  ...
  data() {
    return {
      ...
      total: 0,
      tableData: [],
      searchList: this.yzy.initFilterSearch(['订单编号','文件名'], ['order_num','fileName'])
    }
  },
  mounted() {
    that = this;
  ...
  methods: {
    ...
    getList() {
      ...
      this.axios.post('/api/index/getPrint', param).then((res)=> {
        if (res.data.success) {
    ...
    getList1(param) {
      param.append("aId",sessionStorage.getItem('a_id'))
      this.axios.post('/api/index/like1', param).then((res)=> {
        if (res.data.success) {
          that.tableData = res.data.data
          that.total = res.data.data.length
        } else {
    ...
    search() {
      ...
      if (this.searchList[1].value != '') {
        param.append("fileName",this.searchList[1].value)
      }
      that.getList1(param)
    },
    clear() {
      this.searchList = this.yzy.initFilterSearch(['订单编号','文件名'], ['orderNum','fileName'])
      that.getList()
    },
